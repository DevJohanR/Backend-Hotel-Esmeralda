...
function initializeSocketServer(httpServer) {
  ...
  socketIOInstance.on("connection", (socket) => {
    ...
    socket.on("clienteNotification", (data) => {
      console.log(
        `El administrador recibió una notificación para unirse a la sala del cliente: ${data.clienteId}`
      );
      socketIOInstance
        .to("Administrador")
        .emit("clienteNotification", { clienteId: data.clienteId });
    });
    ...
    socket.on("joinAdminToClientRooms", () => {
      ...
      console.log(
        "El administrador se ha unido a todas las salas de clientes."
      );
    });
    ...
    socket.on("joinAdministrador", () => {
      ...
      socket.emit("administradorJoined", {
        message: 'Administrador se ha unido a la sala "Administrador"',
      });
      socketIOInstance.to("Administrador").emit("adminJoined", {
        message: 'Administrador se ha unido a la sala "Administrador"',
      });
    });
    ...
    socket.on("cliente_mensaje", (data) => {
      console.log(
        `Mensaje recibido al servidor del cliente ${data.clienteId}: ${data.mensaje}`
      );
      socketIOInstance.to("Administrador").emit("admin_mensaje", {
    ...
    socket.on("bookNotification", (bookingData) => {
      socketIOInstance
        .to("Administrador")
        .emit("adminNotification", bookingData);
    ...
    socket.on("mensaje_servidor", (data) => {
      ...
      socketIOInstance.to(data.clienteId).emit("mensaje_cliente", {
        clienteId: data.clienteId,
        mensaje: data.mensaje,
      });
    });
