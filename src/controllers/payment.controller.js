...
const createSession = async (req, res) => {
  try {
    const { userId, services, totalPrice, userEmail, userName } = req.body; // Asumimos que userEmail y userName se envían en el body
    ...
    const reservationDetails = JSON.stringify({ services, totalPrice, reservation_number: session.id }); // Asumiendo que el ID de sesión es el número de reserva
    await sendReservationConfirmationEmail({ username: userName, email: userEmail, reservationDetails });
