...
const createReservation = async (req, res, next) => {
    try {
       ...
       const conflictingReservations = await reservations.findAll({
         where: {
           room_id,
           [Op.or]: [
             {
       ...
       if (conflictingReservations.length > 0) {
         return res.status(400).json({ message: 'La habitación no está disponible para las fechas solicitadas' });
       }
